...
#   - regime dummies: D_2017 and D_2023;
...
#
# Validated model expected by the script: ARDL(3,1,1,1,4)
#
...
Raiz <- function() {
  ...
  if (length(MISSING_PACKAGES)) {
    stop(
      ...
      ". Install them before running ARDL_SA.R.",
      call. = FALSE
    )
  }
  ...
  # ---------- CONFIG GERAL ----------
  PLANTA       <- "SA"
  INPUT_XLSX   <- file.path("data", "02_ardl", "LN_Variaveis_Ajustadas_X13_SA.xlsx")
  SHEET        <- "Ajustada"
  ...
  STRESS_MAP <- list(
    "LN_Temperatura_SA_X13" = +10,
    "LN_Vazao_SA_X13"       = -10,
    "LN_Queimadas_SA_X13"   = +10,
    "LN_560002_SA_X13"      = -10,
    "LN_359001_SA_X13"      = -10
  )
  DEFAULT_PCT <- -10
  ...
  # ============================================================
  # >>> PATCH: DUMMIES DE REGIME (2017-02 e 2023-06) <<<
  # ============================================================
  BREAK_2017 <- as.Date("2017-02-01")
  BREAK_2023 <- as.Date("2023-06-01")
  
  DUMMY_COLS <- c("D_2017", "D_2023")
  ...
  cat("== ARDL SA reproducibility pipeline ==\n")
  ...
  # ============================================================
  # PARSER FOR dynlm L(...) TERM NAMES — VALIDATED
  # For range expansions, the suffix is the actual lag value:
  #   L(x,0:1)0 -> lag 0
  #   L(x,0:1)1 -> lag 1
  # ============================================================
  ...
  # ============================================================
  # >>> PATCH: CRIAÇÃO DAS DUMMIES (STEP) 2017-02 e 2023-06 <<<
  # ============================================================
  df[[date_col]] <- as.Date(df[[date_col]])
  df$D_2017 <- as.integer(df[[date_col]] >= BREAK_2017)
  df$D_2023 <- as.integer(df[[date_col]] >= BREAK_2023)
  
  cat("\n== Dummies de regime inseridas ==\n")
  cat("D_2017 = 1 se Data >= ", as.character(BREAK_2017), " (inclusive)\n", sep = "")
  cat("D_2023 = 1 se Data >= ", as.character(BREAK_2023), " (inclusive)\n", sep = "")
  cat("Prévia contagens:\n")
  print(data.frame(
    Dummy = c("D_2017","D_2023"),
    Soma1 = c(sum(df$D_2017, na.rm=TRUE), sum(df$D_2023, na.rm=TRUE)),
    Soma0 = c(sum(1L - df$D_2017, na.rm=TRUE), sum(1L - df$D_2023, na.rm=TRUE))
  ), row.names = FALSE)
  ...
  EXPECTED_ORDER <- c(3L, 1L, 1L, 1L, 4L)
  ...
  if (!identical(XS_ORDERED, EXPECTED_XS)) {
    stop("FINAL_VALID_200826: selected X variables differ from the validated SA baseline.")
  }
  if (!identical(unname(CURRENT_ORDER), EXPECTED_ORDER)) {
    stop("FINAL_VALID_200826: lag order differs from validated ARDL(3,1,1,1,4).")
  }
  ...
  keep <- grepl("LN_Vazao_SA_X13", tn, fixed = TRUE)
  print(data.frame(
    Termo = tn[keep],
    Beta  = unname(b[keep]),
  ...
  bk0 <- .get_beta_k0(fit, "LN_Vazao_SA_X13")
  ...
  if (!isTRUE(top3_audit$Top3_Identical[1])) {
    stop("FINAL_VALID_200826: corrected parser changed the validated SA combined-stress Top-3.")
  }
  ...
  # ============================================================
  cat("\n== Bounds test aligned with D_2017/D_2023 (case=", CASE_BOUNDS, ") ==\n", sep="")
  ...
  validation_cfg <- data.frame(
    ...
    Valor = c(
      ...
      "dynlm expanded-range suffix equals actual lag",
      "TRUE",
      STRESS_MODE,
      "ARDL(3,1,1,1,4)"
    ),
  ...
  dummies_info <- data.frame(
    ...
    Inicio = c(as.character(BREAK_2017), as.character(BREAK_2023)),
    Regra = c("1 se Data >= 2017-02-01; 0 caso contrário",
              "1 se Data >= 2023-06-01; 0 caso contrário"),
    check.names = FALSE
  )
  ...
  make_word_report <- function(results, docx_path){
    ...
    tb_meta <- data.frame(
      Item = c("PLANTA","RUN_ID","y","xs_final","top3_comb","best_order","ROBUST_MODE","NW_LAG","NW_PREWHITE","NW_ADJUST","BREAK_2017","BREAK_2023"),
      Valor = c(
        results$meta$PLANTA,
        results$meta$RUN_ID,
        results$meta$y,
        ...
        as.character(results$meta$dummies$break_2017),
  ...
  SESSION_INFO_PATH <- file.path(BASE_DIR, "logs", "sessionInfo_SA_reproduced.txt")
